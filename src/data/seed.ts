...
const agents: Agent[] = [
  agent({
    ...
    responsibilities: ["Coordinates the complete workflow", "Routes work to specialist agents", "Enforces human approval gates", "Never approves gates, performs AMBER/RED actions, or decides client scope"],
  ...
  agent({
    ...
    shortCode: "02",
    name: "Site Architect",
    role: "Sitemap, site architecture, UX, journeys, conversion architecture, navigation, keyword-to-page strategy, catalogue/ecommerce architecture",
    responsibilities: [
      ...
      "Catalogue & ecommerce architecture",
  ...
  agent({
    ...
    role: "Visual direction, layout system, design tokens, imagery — operates in DIRECTION, COMPOSITION, and VISUAL_REVIEW passes",
    responsibilities: [
      "DIRECTION pass: brand and competitive direction analysis",
      "COMPOSITION pass: page composition and layout design",
      "VISUAL_REVIEW pass: screenshot-based visual validation (requires real screenshots — never DOM or text crawl alone)",
      "Design tokens",
      "Imagery direction",
    ],
    status: "IDLE",
  ...
  agent({
    ...
    shortCode: "04",
    name: "Content",
    role: "Page copy, metadata, schema content, alt text, internal linking, brand voice",
    responsibilities: [
      "Page copy",
      "Metadata (title, description, OG tags)",
      "Schema content (structured data text)",
      "Alt text for images",
      "Internal linking",
  ...
  agent({
    ...
    shortCode: "06",
    name: "QA",
    role: "Independent QA — technical, visual, responsive, journey, content, SEO, and launch-readiness evidence. Does not own or execute launch.",
    responsibilities: [
      "Technical QA",
      "Visual QA",
      "Responsive QA",
      ...
      "Content QA",
      "SEO QA",
      "Launch-readiness evidence and audit",
      "Defect detection and severity classification",
      "Challenge prior agent claims",
    ],
    status: "IDLE",
    outputsProduced: 2,
    ...
    canExecuteSiteChanges: false,
    mission: "Detect defects across every QA dimension and produce evidence of launch readiness — never owns or executes the launch itself.",
    exclusions: ["Never executes or approves launch (Agent 07 owns launch runbook, humans approve)", "Never certifies its own audit output without real evidence"],
  }),
  agent({
    id: AGENT_IDS.A07,
    code: "A07",
    ...
    role: "Launch runbook, infrastructure verification, migration/preflight preparation, hosting/domain/DNS readiness, backup/rollback, post-launch verification, controlled activation preparation",
    responsibilities: [
      "Launch runbook",
      "Infrastructure verification",
      "Migration and preflight preparation",
      "Hosting, domain and DNS readiness",
    ...
    canExecuteSiteChanges: true,
    mission: "Prepare and verify every infrastructure condition required for a safe launch — never autonomously executes RED actions (DNS, registrar, payment) without explicit human authorisation.",
    exclusions: ["Never autonomously executes RED actions (DNS, nameservers, payment credentials, registrar) without explicit named human authorisation"],
  }),
  agent({
    id: AGENT_IDS.A08,
    code: "A08",
    shortCode: "08",
    name: "Curator",
    role: "Event-triggered or scheduled review of evidence — proposes lesson candidates, never runs continuously",
    responsibilities: [
      "Review client feedback (event-triggered on feedback receipt)",
      "Review QA defects (event-triggered on QA run completion)",
      "Identify successful & failed patterns (scheduled post-project review)",
      "Review performance & conversion evidence (scheduled)",
      "Propose lesson candidates",
    ],
    status: "IDLE",
    outputsProduced: 0,
    ...
    canExecuteSiteChanges: false,
    mission: "Turn scattered evidence (QA defects, client feedback, performance data) into proposed, evidence-backed lessons. Runs on trigger (feedback/QA events) or schedule (post-project) — never autonomously and continuously.",
    exclusions: ["Never approves its own lesson proposals (a human always reviews)", "Never builds or executes site changes", "Never runs continuously — only on event trigger or schedule"],
...
export const seedData: OSData = {
  ...
  externalClients: [],
  externalAccessLog: [],
  // CTOS-005A additions — empty arrays; populated at runtime
  buildPacks: [],
  revisionRounds: [],
  changeRequests: [],
  clientAssets: [],
